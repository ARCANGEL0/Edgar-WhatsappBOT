...
export async function before(m, { conn, isAdmin, isBotAdmin, usedPrefix }) {
  ...
  let fakemek = {key: {participant: "0@s.whatsapp.net","remoteJid": "0@s.whatsapp.net"},"message": {"groupInviteMessage": {"groupJid": "51995386439-1616969743@g.us","inviteCode": "m","groupName": "P", "caption": 'ɢᴀᴛᴀʙᴏᴛ-ᴍᴅ', 'jpegThumbnail': null}}}
   if (chat.antiTraba && m.text.length > 4000) { //Cantidad máxima de caracteres aceptados en un mensaje//
    if (isAdmin) return conn.sendMessage(m.chat, { text: `${lenguajeGB['smsEnlaceWat']()} administrador @${m.sender.split("@")[0]} acaba de enviar un texto que contém muitos caracteres -.-!`, mentions: [m.sender] }, { quoted: fakemek })
    conn.sendMessage(m.chat, `*[ ! ] Detectou-se  uma mensagem que contem muitos caracteres [ ! ]*\n`, `${isBotAdmin ? '' : 'Sinto que me falta direitos de administrador, não posso fazer nada além de contemplar em minha solidão:/'}`, m)
    //await conn.sendButton(m.chat, `*[ ! ] Se detecto un mensaje que contiene muchos caracteres [ ! ]*\n`, `${isBotAdmin ? '' : 'No soy administrador, no puedo hacer nada :/'}`, author, ['[ ᴅᴇsᴀᴄᴛɪᴠᴀ ᴀɴᴛɪᴛʀᴀʙᴀ ]', usedPrefix+'apagar antitraba'], fakemek )
        ...
        conn.sendMessage(m.chat, { delete: { remoteJid: m.chat, fromMe: false, id: bang, participant: delet }})
        	setTimeout(() => { 
        	conn.sendMessage(m.chat, { text: `Marcar chat como lido ✓\n${"\n".repeat(400)}\n• O número : wa.me/${m.sender.split("@")[0]}\n• Alias : ${name}\n‼️Acaba de enviar um texto que contém muitos caracteres que pode ocasionar falhas nos dispositivos`, mentions: [m.sender] }, { quoted: fakemek })
        }, 0)
        setTimeout(() => { 
